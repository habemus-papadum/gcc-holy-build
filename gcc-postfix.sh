...
## Based off of https://raw.githubusercontent.com/stuarteberg/conda-recipes/52dc383192ed824cd4a8faca5da10e4785623c42/gcc-4.8/post-link.sh
...
SYSTEM_GCC=/opt/rh/devtoolset-2/root/usr/bin/gcc
...
for library_path in $(ld --verbose | grep SEARCH_DIR | sed -r 's/SEARCH_DIR\("=?([^"]*)"\);/ \1/g'); do
    for obj_file in $C_RUNTIME_OBJ_FILES; do
        ...
        if [[ -e "$obj_file_full_path" ]]; then
            cp "$obj_file_full_path" "${PREFIX}/lib/gcc/"*/*/
